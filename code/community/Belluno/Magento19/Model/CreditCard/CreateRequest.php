...
class Belluno_Magento19_Model_CreditCard_CreateRequest {
  ...
  public function createRequest($data, $info) {
    ...
    $cardHash = $this->getEncrypt()->encrypt($cardNumber, $data['card_month_exp'], $data['card_year_exp'], $data['cc_cvv']);
  ...
  public function getRuleCapture() {
    $capture = Mage::getStoreConfig('payment/belluno_creditcardpayment/payment_action');
    if ($capture == 'authorize_capture') {
      return 1;
    } else {
      return 2;
    }
  ...
  /**Function to return class encrypt */
  public function getEncrypt() {
    return new Belluno_Magento19_Validations_Encrypt();
  }
